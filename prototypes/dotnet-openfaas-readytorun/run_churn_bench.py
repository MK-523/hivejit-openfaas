#!/usr/bin/env python3
"""Run sequential C#/.NET OpenFaaS invocations with real pod churn."""
...
def request_url(args: argparse.Namespace, invocation: int) -> str:
    params = {
        "scenario": args.scenario,
        "iterations": str(args.iterations),
        "seed": str(args.seed + invocation),
    }
    return f"{args.gateway.rstrip('/')}/function/{args.function}/work?{urllib.parse.urlencode(params)}"
...
def main() -> int:
    ...
    parser.add_argument("--churn-at", default="1,112,478,800,1044,1283,1679,1790")
    ...
    for invocation in range(1, args.invocations + 1):
        ...
        status, http_latency_ms, body, error = invoke(request_url(args, invocation), args.invoke_timeout)
        ...
        row = {
            ...
            "pod": pod,
            "pod_uid": payload.get("podUid") or pod_uid,
            "restart_ms": f"{restart_ms:.6f}" if churn else "",
            "http_latency_ms": f"{http_latency_ms:.6f}",
            "handler_elapsed_ms": f"{as_float(payload.get('elapsedMs')):.6f}",
            "handler_p50_ms": f"{as_float(payload.get('p50Ms')):.6f}",
            "handler_p95_ms": f"{as_float(payload.get('p95Ms')):.6f}",
            "process_uptime_ms": f"{as_float(payload.get('processUptimeMs')):.6f}",
            "request_in_pod": as_int(payload.get("requestInPod")),
            "mode": payload.get("build") or args.mode,
            "status": status,
